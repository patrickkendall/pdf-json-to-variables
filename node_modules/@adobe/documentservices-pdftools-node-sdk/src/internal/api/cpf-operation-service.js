...
class CpfOperationService {
	...
	perform(context, operationMessage) {
		let cpfMessage = this.getCPFMessage(operationMessage),
			...
			contentAnalyzerRequests = new CpfContentAnalyzerRequests(engine,cpfMessage.inputDocuments,cpfMessage.params,cpfMessage.outputDocument),
		...
		return CPFApi.cpfCreateApi(context, cpfMessage.files, contentAnalyzerRequestsJsonString, operation)
			...
			.then(targetFileRef => {
				...
				if(!Array.isArray(targetFileRef) && operationMessage.operationName === DefaultConfig.operationName.splitPDF){
